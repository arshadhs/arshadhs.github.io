...
flowchart TD
    ...
    DRL --> F["Foundations"]
    DRL --> MDP["Markov Decision Process"]
    DRL --> SOLVE["Ways to Solve RL Problems"]
    DRL --> DEEP["Deep RL Approaches"]

    %% --------------------
    %% Foundations
    %% --------------------

    F --> RL["Reinforcement Learning Basics"]
    F --> MAB["Multi-Armed Bandits"]
    ...
    MAB --> EG["ε-Greedy"]
    MAB --> UCB["Upper Confidence Bound"]
    MAB --> SAV["Sample Average"]
    MAB --> CA["Constant α"]
    MAB --> OIV["Optimistic Initial Values"]
    MAB --> GB["Gradient Bandit"]
    MAB --> CB["Contextual Bandit"]
    ...
    MDP --> ST["States"]
    MDP --> AC["Actions"]
    MDP --> TP["Transition Probabilities"]
    MDP --> RW["Rewards"]
    MDP --> RET["Returns"]
    MDP --> POL["Policy π"]
    MDP --> VF["Value Functions"]
    MDP --> BE["Bellman Equations"]

    VF --> VS["Vπ(s): State Value"]
    VF --> QS["Qπ(s,a): State-Action Value"]

    BE --> BEE["Bellman Expectation Equation"]
    BE --> BEO["Bellman Optimality Equation"]

    BEE --> EVAL["Evaluate a Given Policy"]
    BEO --> OPT["Find Best / Optimal Behaviour"]

    %% --------------------
    %% Solving RL
    %% --------------------

    SOLVE --> MB["Model-Based"]
    SOLVE --> MF["Model-Free"]
    ...
    DP --> PIMP["Policy Improvement"]
    DP --> PITER["Policy Iteration"]
    DP --> VITER["Value Iteration"]
    DP --> ASYNC["Asynchronous Value Iteration"]

    PITER --> PI1["Evaluate Policy"]
    PI1 --> PI2["Improve Policy"]
    PI2 --> PI1

    VITER --> VI1["Bellman Optimality Backup"]
    VI1 --> VI2["Repeat Until Δ < θ"]
    VI2 --> VI3["Extract Policy"]
    ...
    MC --> MCC["MC Control"]
    MC --> FV["First-Visit MC"]
    MC --> EV["Every-Visit MC"]
    MC --> ONMC["On-Policy MC"]
    MC --> OFFMC["Off-Policy MC"]
    OFFMC --> IS["Importance Sampling"]
    ...
    DEEP --> VBA["Value-Based"]
    DEEP --> PBA["Policy-Based"]
    DEEP --> AC2["Actor-Critic"]
    DEEP --> FA["Function Approximation"]

    FA --> NN["Neural Networks"]

    VBA --> DQN["DQN"]
    PBA --> PG["Policy Gradient"]
    AC2 --> MIX["Value + Policy"]
    ...
    classDef deep fill:#EDE7F6,stroke:#7E57C2;

    class DRL,MDP main;
    class MB,MF model;
    class DP,MC,TD method;
    class DEEP,VBA,PBA,AC2,FA,DQN,PG deep;
{{< /mermaid >}}
